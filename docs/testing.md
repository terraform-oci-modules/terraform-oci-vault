...
- A live apply/destroy of a `DEFAULT` vault with an `AES`, `RSA`, and `ECDSA`
  key was run against a real tenancy on 2026-09-18 and completed cleanly end to
  end (all resources scheduled for deletion on `terraform destroy`, no errors).
